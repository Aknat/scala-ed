...
object HomeTask extends App {
  ...
  val a: PaymentDomain = {
    case r =>
      ...
      PaymentResult(r, resp)
  }

  val paymentsProcessing = new PartialFunction[PaymentRequest, PaymentResult] {
    override def isDefinedAt(req: PaymentRequest): Boolean =
      serverPool.exists(_.canProcess(req.productType))

    override def apply(req: PaymentRequest): PaymentResult = {
      val paymentService = serverPool.find(_.canProcess(req.productType))
//      println(paymentService)
      paymentService.map(s => s.withdrawPayment(req, req => "null")) match {
        case Some(r) => PaymentResult(req, r)
      }
    }
  }
  ...
  def result(seq: Seq[PaymentRequest]): Seq[PaymentStatus] = {
    val paymentResponses = seq.map(paymentsProcessing orElse ({
      case r => PaymentResponse(400, 1)
    }))
    paymentResponses.foreach(println)
    Seq()
  }
  ...
  1) You have to call method canProcess[T <: Product](product: T): Boolean in PaymentService to ensure operation execution. This method is not costly due to it cashing nature
